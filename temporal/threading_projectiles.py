...
def create_projectile(index, projectiles, barrier):
    """Función para instanciar un proyectil en un hilo."""
    projectiles[index] = Projectile(WIDTH, HEIGHT, x_reference, y_reference)
    barrier.wait()  # Esperar a que todos los hilos terminen antes de seguir
...
def main():
    ...
    projectiles = [None] * num_projectiles
    barrier = threading.Barrier(num_projectiles + 1)  # +1 para incluir el hilo principal

    # Crear hilos para instanciar proyectiles
    threads = []
    for i in range(num_projectiles):
        t = threading.Thread(target=create_projectile, args=(i, projectiles, barrier))
        threads.append(t)
        t.start()

    barrier.wait()  # Espera a que todos los proyectiles se hayan creado
